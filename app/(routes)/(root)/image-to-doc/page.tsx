...
const ImageToDoc = () => {
    ...
    const downloadFile = async () => {
        ...
        const file = new File([blob], "extracted_data.docx", {type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document"});

        const url = window.URL.createObjectURL(blob);

        await axios.patch(`/api/upload?fileKey=${fileKey}`, {file   }, {headers: {"Content-Type": "multipart/form-data"}});
                ...
                {text && documentContent && (
                    <Button className="bg-primary text-white" onClick={downloadFile}>
                        {loading ? (
                            ...
                            "Save for later and Download Document"
                        )}
                    </Button>
                )}
            </div>
